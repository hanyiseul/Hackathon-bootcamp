    ...
    <div className="admin-layout">
      <aside className="sidebar">
        <div className="side-brand">
          <div className="brand-logo">PT</div>
          <div>
            <strong>PTTime | PT 예약 관리 시스템</strong>
      ...
      <div className="workspace">
        <header className="topbar">
          <div>
            <p className="crumb">PT 예약 관리 / 관리자</p>
            <h1>PT 예약 관리 대시보드</h1>
          </div>

          <div className="top-actions">
            <button className="ghost" onClick={() => dashboard.refetch()}>
              새로고침
            </button>
        ...
        <main className="content">
          <section className="hero-card">
            <div>
              <span className="hero-label">PT Reservation Management</span>
              <h2>회원과 예약 정보를 한눈에 관리합니다.</h2>
              <p>
                회원 관리, 트레이너 관리, 예약 현황을 실시간으로 확인할 수 있습니다.
              </p>
            </div>
          ...
          <section className="metric-grid">
            <div className="metric-card">
              <span>회원</span>
              <strong>{data.userCount ?? users.length}</strong>
              <p>전체 회원</p>
            </div>

            <div className="metric-card">
              <span>트레이너</span>
              <strong>{data.accountCount ?? accounts.length}</strong>
              <p>등록된 트레이너</p>
            </div>

            <div className="metric-card">
              <span>예약 건수</span>
              <strong>{data.transactionCount ?? transactions.length}</strong>
              <p>전체 예약 현황</p>
            </div>
          ...
          <section className="operation-grid">
            <div className="panel">
              ...
              <button onClick={() => createUser.mutate(user)}>
                회원 등록
              </button>
            ...
            <div className="panel">
              ...
              <input
                ...
                onChange={(e) =>
                  setAccount({ ...account, userId: Number(e.target.value) })
                }
              />

              <input
                placeholder="PT 이용 횟수"
              ...
              <button onClick={() => createAccount.mutate(account)}>
                이용권 등록
              </button>
        ...
        <footer className="app-footer">
          <span>PT 예약 관리 시스템</span>
          <span>Admin Console · Spring Boot · MariaDB · Redis</span>
        </footer>
      </div>
    </div>
  );
