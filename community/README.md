...
* JWT 인증 기반 로그인 시스템 구현
* REST API 기반 게시판 CRUD 구현
* 회사 기반 게시판 분리 기능 구현
...
* 로그인 토큰 검증 로직에서 초기 설계가 꼬여 인증 처리에 어려움 발생
* 일부 페이지에서 데이터 구조와 DOM 구조가 섞여 디버깅 시간이 증가
...
* JWT 기반 인증 처리 방식 이해
* SQL JOIN을 활용한 데이터 조회 방식 학습
* Node.js 서버에서 API 라우팅 구조 설계 경험
